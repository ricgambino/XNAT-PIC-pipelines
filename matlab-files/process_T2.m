...
% 16/04/2019
% 
function process_T2(input_img_dir,output_img_dir)
    ...
    % T2 map figure
    h_T2_map = figure('Name', 'T2 map [ms]');
    T2_map = imshow(T2_image, []);
    axis square   
    colorbar
    title('T2 map [ms]')
    saveas(h_T2_map, [folder_name '_T2_map.jpg']);
    ...
    T2_nii = make_nii(T2_image(:,:,:), voxel_size, origin, datatype, '');
    T2_nii_or = rri_orient_no_ui(T2_nii);
...
fprintf(fid,'----------------------------------------------------------------\n');
fprintf(fid, 'CIM-XNAT Pipeline: Process T2 images\n');
...
% save variables
save([folder_name '_T2_map' '_variables.mat' ])
